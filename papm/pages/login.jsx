  ...
  const handleSubmit = async (e) => {
    ...
    if (isValid) {
      try {
        ...
        if (user.message === "loginUserFailed") {
          setMessage("Login Failed");
          location.reload();
        }

        if (user.message === "loginUserSuccess") {
          setIsLoggedIn(true);
        ...
        <div className="w-full h-full flex flex-col justify-center items-center backdrop-blur-lg">
          ...
          >
            ...
            <button
              ...
              className="w-96 px-7 py-2 mb-2 font-oswald bg-noir-200 bg-opacity-80 text-blanc-50 hover:text-or-200 focus:text-or-50 opacity-100 rounded-md sm:inline-block"
            >
              Sign In
            </button>
